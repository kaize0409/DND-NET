...
def run_main():
    ...
    print(f"Running Experiment on {args.dataset} with {args.noise} noise at rate {args.ptb_rate} and label rate {args.label_rate}")
    ...
    degrees = pyg_utils.degree(data.edge_index[0])
    args.max_n = degrees.max().to(torch.long)
    ind_nid_m, ind_m, sim_score_m, nb_m = pre_compute_local_sim_(g, data.edge_index, args, max_neighbor=args.max_n)

    data.ind_nid_m, data.ind_m, data.nb_m = ind_nid_m.detach(), ind_m.detach(), nb_m.detach()
    data.sim_score_m = sim_score_m.detach()
    ...
    for run in range(1, args.runs+1):
        ...
        acc_val_all, acc_test_all = train_DndNet(model, optimizer, g, data, idx_train, idx_val, idx_test, labels, args)
